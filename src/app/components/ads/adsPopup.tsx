...
export default function AdPopup() {
  ...
  return (
    <div className="fixed left-4 right-4 bottom-4 z-[1000] bg-[#97d5c9] border shadow-xl p-3 sm:p-4 flex flex-col sm:flex-row gap-3 sm:gap-4 animate-fadeIn max-w-[90vw] sm:max-w-5xl sm:ml-4 sm:mr-auto mx-auto rounded-lg overflow-hidden">
      <Button
        onClick={handleClose}
        variant="outline"
        aria-label="Stäng"
        className="absolute top-2 right-2 sm:top-3 sm:right-3 h-8 w-8 p-0 text-xs font-semibold uppercase tracking-wide hover:underline cursor-pointer"
      >
        ✕
      </Button>

      {/* Image — much smaller on sm and aligned left */}
      <div className="relative w-full aspect-[16/9] max-h-40 overflow-hidden rounded-md
                      sm:basis-1/5 sm:aspect-[4/3] sm:max-h-24 sm:self-start
                      md:aspect-[16/9] md:max-h-44 lg:basis-1/2 lg:max-h-56">
        ...
        {logo?.sourceUrl ? (
          <Image
            ...
            sizes="(min-width: 1024px) 50vw, (min-width: 640px) 20vw, 100vw"
            priority
          />
        ) : (
      ...
      <div className="flex-1 min-w-0 flex flex-col justify-between">
        <div>
          ...
          {/* Smaller text specifically on sm */}
          <h3 className="text-base sm:text-sm md:text-2xl font-extrabold text-gray-900 leading-snug mb-1">
